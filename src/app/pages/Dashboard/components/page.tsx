...
const DashboardPage = () => {
  ...
  const today = dayjs();
  const dayOfWeek = today.day(); // 0: domingo
  const monday = today.subtract(dayOfWeek === 0 ? 6 : dayOfWeek - 1, 'day').startOf('day');
  const saturday = monday.add(5, 'day').endOf('day'); // sábado

  const citasSemana = appointments?.filter((a) => {
    const citaDate = dayjs(`${a.date} ${a.time}`, "YYYY-MM-DD HH:mm:ss"); // Combinar fecha y hora
    return citaDate.isAfter(monday.clone().subtract(1, 'minute')) && citaDate.isBefore(saturday.clone().add(1, 'minute'));
  }) || [];
  return (
    <div className="p-8 min-h-screen bg-blue-50 transition-all">
      ...
      <div className="flex justify-between items-center mb-10">
        <div>
          ...
          <p className="text-lg text-gray-700 mt-2">
            Tienes {appointmentsCount} citas pendientes  y {seguimientos.pendiente} seguimientos pendientes.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-12">
        <MetricCard title="Usuarios" value={usersCount} subtitle="+5 esta semana" icon={<FaUsers />} />
        <MetricCard title="Roles" value={rolesCount} subtitle="+1 nuevo" icon={<FaUserShield />} />
        <MetricCard title="Citas Pendientes" value={appointmentsCount} subtitle="Próxima 14:00h" icon={<FaCalendarAlt />} />
        <MetricCard title="Seguimientos" value={totalSeguimientos} subtitle={`${seguimientos.completado} completados`} icon={<FaChartLine />} />
      </div>

      {/* Mini calendario */}
      <div className="mb-12">
        <MiniCalendar citas={citasSemana} semanaDesde={monday.format('DD/MM/YYYY')} semanaHasta={saturday.format('DD/MM/YYYY')} />
      </div>
